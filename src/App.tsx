...
function App() {
  ...
  const [categories, setCategories] = useState<string[]>([]); // 🔥 이제 DB에서 가져옵니다
  ...
  // 🔥 1. 기기 간 동기화의 핵심: 비디오 데이터에서 실제 사용 중인 카테고리만 뽑아냅니다.
  const fetchVideos = async () => {
    const { data, error } = await supabase.from('videos').select('*').order('created_at', { ascending: false });
    if (!error && data) {
      setVideos(data);
      // 현재 영상들이 가진 카테고리들을 중복 없이 추출하여 목록 생성 (어떤 기기든 동일하게 보임)
  ...
  useEffect(() => {
    fetchVideos();
    if (window.innerWidth < 768) setViewMode('small');
  }, []);
  ...
  // 카테고리 이름 변경 (DB를 직접 건드리므로 모든 기기 동기화됨)
  const handleEditCategory = async (oldName: string) => {
    const newName = window.prompt(`'${oldName}'의 새 이름을 입력하세요:`, oldName);
    if (newName && newName.trim() && newName !== oldName) {
      const { error } = await supabase.from('videos').update({ category: newName.trim() }).eq('category', oldName);
      if (!error) {
        fetchVideos();
  ...
  // 카테고리 삭제 (해당 카테고리 영상들을 'All' 또는 다른 곳으로 옮기거나 삭제 처리)
  const handleDeleteCategory = async (e: React.MouseEvent, catToDelete: string) => {
    e.stopPropagation(); 
    if (window.confirm(`'${catToDelete}' 카테고리의 모든 데이터를 삭제하시겠습니까?`)) {
      const { error } = await supabase.from('videos').delete().eq('category', catToDelete);
      if (!error) fetchVideos();
    }
  };

  const currentBreakpoints = viewMode === 'large' 
    ? { default: 5, 1536: 5, 1280: 4, 1024: 3, 768: 2, 640: 1 } 
    : { default: 10, 1536: 10, 1280: 8, 1024: 6, 768: 4, 640: 2 };
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#fff0f5] via-white to-[#f0f8ff] text-black w-full overflow-x-hidden">
      <div className="max-w-[1920px] mx-auto px-4 sm:px-8 pt-5 pb-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div onClick={() => { setSelectedCategory('All'); setSearchQuery(''); }} className="flex items-center gap-4 cursor-pointer">
          <img src={logoImg} alt="Logo" className="h-12 md:h-16 w-auto" />
          <h1 className="text-5xl md:text-7xl font-black tracking-tighter">SCRAP ROE</h1>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setIsAddOpen(true)} className="px-6 py-2 bg-[#FF66C4] text-white rounded-full font-bold shadow-md">Add</button>
      ...
      <div className="max-w-[1920px] mx-auto px-4 sm:px-8">
        <div className="flex gap-2 mb-4">
          <div className="relative flex-1 h-11 md:h-12">
            ...
            <input type="text" placeholder="검색..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full h-full pl-12 pr-10 border-2 border-black rounded-full outline-none" />
          ...
          <div className="flex border-2 border-black rounded-full p-1 h-11 md:h-12 bg-white shrink-0">
            <button onClick={() => setViewMode('large')} className={`px-4 rounded-full text-xs font-bold ${viewMode === 'large' ? 'bg-[#FF66C4] text-white' : ''}`}>■ 크게</button>
            <button onClick={() => setViewMode('small')} className={`px-4 rounded-full text-xs font-bold ${viewMode === 'small' ? 'bg-[#FF66C4] text-white' : ''}`}>▦ 작게</button>
          </div>
        </div>

        <div className="flex flex-wrap gap-2 mb-8">
          <button onClick={() => setSelectedCategory('All')} className={`px-5 py-2 rounded-full font-bold border-2 ${selectedCategory === 'All' ? 'bg-pink-100 border-pink-200 text-pink-600' : 'bg-white border-gray-300'}`}>All</button>
          {categories.map(cat => (
            <button key={cat} onClick={() => isCategoryEditMode ? handleEditCategory(cat) : setSelectedCategory(cat)} className={`relative px-5 py-2 rounded-full font-bold border-2 ${selectedCategory === cat ? 'bg-pink-100 border-pink-200 text-pink-600' : 'bg-white border-gray-300'} ${isCategoryEditMode ? 'border-dashed border-pink-400 pr-10 animate-pulse' : ''}`}>
              {cat}
